...
export function StartupTemplate({ state, isPro }: TemplateProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="w-3 flex-shrink-0 rounded-l-lg -ml-8 -my-8"
        style={{ backgroundColor: brand, marginLeft: '-32px', marginTop: '-32px', marginBottom: '-32px' }}
      />
      ...
      <div className="flex-1 pl-8">
        ...
        <div className="flex gap-8 mb-8">
          <div className="px-4 py-2 rounded-lg bg-gray-50">
            ...
            <p className="font-semibold">{state.document.date}</p>
          </div>
          <div className="px-4 py-2 rounded-lg bg-gray-50">
            <p className="text-xs text-gray-400 uppercase tracking-wider">Due</p>
            <p className="font-semibold">{state.document.dueDate}</p>
          </div>
        </div>
        ...
        <table className="w-full mb-8" style={{ fontSize }}>
          <thead>
            <tr style={{ borderBottom: `2px solid ${brand}` }}>
              <th className="text-left py-2 text-xs uppercase tracking-wider font-bold w-8" style={{ color: brand }}>#</th>
              <th className="text-left py-2 text-xs uppercase tracking-wider font-bold" style={{ color: brand }}>Description</th>
              <th className="text-right py-2 text-xs uppercase tracking-wider font-bold w-14" style={{ color: brand }}>Qty</th>
              <th className="text-right py-2 text-xs uppercase tracking-wider font-bold w-14" style={{ color: brand }}>Unit</th>
              <th className="text-right py-2 text-xs uppercase tracking-wider font-bold w-24" style={{ color: brand }}>Rate</th>
              <th className="text-right py-2 text-xs uppercase tracking-wider font-bold w-24" style={{ color: brand }}>Amount</th>
            </tr>
          </thead>
          <tbody>
            {state.items.map((item, idx) => {
              if (item.type === 'heading') {
                return (
                  <tr key={item.id}>
                    <td colSpan={6} className="py-2 font-bold text-gray-700 border-b border-gray-100">{item.description}</td>
                  </tr>
                );
              }
              return (
